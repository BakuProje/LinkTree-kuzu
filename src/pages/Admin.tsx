...
const Admin = () => {
  ...
  const handleProfileSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newProfile: ProfileData = {
      ...
      tiktokUrl: profile.tiktokUrl, // Keep existing values but don't edit
  ...
  if (!isAuthenticated) {
    return (
      <div className="relative min-h-screen bg-background text-foreground font-mono-cyber flex items-center justify-center p-4">
        <MatrixRain />
        <div className="relative z-10 w-full max-w-sm glass-card neon-border-blue p-8 text-center animate-in fade-in zoom-in duration-500">
          <div className="w-20 h-20 rounded-full bg-neon-blue/10 border border-neon-blue/50 flex items-center justify-center mx-auto mb-6 shadow-[0_0_20px_rgba(0,255,255,0.2)]">
            <i className="fa-solid fa-user-shield text-3xl text-neon-blue" />
          </div>
          <h2 className="text-2xl font-bold neon-text-blue tracking-tighter mb-2 uppercase">Restricted Access</h2>
          <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] mb-8">System Security Protocol Enabled_</p>
          
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <div className="space-y-2">
              <label className="text-[10px] text-left block text-muted-foreground uppercase tracking-widest ml-1">Admin Identity PIN</label>
              <input 
                type="password"
                maxLength={6}
                value={pin}
                ...
                className="w-full bg-black/60 border border-white/5 rounded-xl p-4 text-center text-2xl tracking-[1em] outline-none focus:border-neon-blue/50 focus:bg-neon-blue/5 transition-all"
                placeholder="******"
                autoFocus
              />
            </div>
            <div className="pt-4">
              <button 
                type="button"
                onClick={() => navigate("/")}
                className="w-full py-2 text-[10px] text-muted-foreground hover:text-foreground transition-colors uppercase tracking-widest"
              >
    ...
    <div className="relative min-h-screen bg-background text-foreground font-mono-cyber overflow-x-hidden">
      ...
      <main className="relative z-10 px-4 pb-20 pt-10 max-w-2xl mx-auto">
        ...
        {activeTab === "links" ? (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {links.map((link, index) => (
                <div key={link.id} className="glass-card neon-border-blue p-4 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="w-10 h-10 shrink-0 rounded border border-white/10 flex items-center justify-center bg-black/40 overflow-hidden">
                      {link.isImage ? (
          ...
          /* Profile Tab */
          <form onSubmit={handleProfileSave} className="space-y-4 glass-card neon-border-purple p-6">
            <div className="space-y-2">
              <label className="text-xs text-neon-purple uppercase tracking-widest font-bold">NAMA PROFIL</label>
              <input 
                name="name"
                defaultValue={profile.name}
                className="w-full bg-black/40 border border-white/10 rounded p-4 text-sm focus:border-neon-purple outline-none transition-all focus:bg-neon-purple/5"
                placeholder="Masukkan nama profil"
              />
            </div>
            <div className="space-y-2">
              <label className="text-xs text-neon-purple uppercase tracking-widest font-bold">BIO / SUBTITLE</label>
              <input 
                name="subtitle"
            ...
            <div className="space-y-2">
              <label className="text-xs text-neon-purple uppercase tracking-widest font-bold">AVATAR IMAGE URL</label>
              <input 
                name="avatar"
                defaultValue={profile.avatar}
                className="w-full bg-black/40 border border-white/10 rounded p-4 text-sm focus:border-neon-purple outline-none transition-all focus:bg-neon-purple/5"
                placeholder="/logo.png atau URL luar"
              />
            </div>
      ...
      {(editingId || isAdding) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-xl glass-card neon-border-blue p-8 space-y-6 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-300">
            ...
            <div className="grid gap-6">
              <div className="space-y-2">
                <label className="text-[10px] text-neon-blue font-bold tracking-widest uppercase">NAMA LINK</label>
                <input 
                  value={editLink.label || ""}
                  onChange={(e) => setEditLink({...editLink, label: e.target.value})}
                  className="w-full bg-black/60 border border-white/5 rounded-lg p-4 text-sm outline-none focus:border-neon-blue/50 focus:bg-neon-blue/5 transition-all text-foreground"
                  placeholder="Misal: Website Portfolio"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] text-neon-blue font-bold tracking-widest uppercase">URL TUJUAN</label>
                <input 
                  value={editLink.href || ""}
              ...
              <div className="space-y-2">
                <label className="text-[10px] text-neon-blue font-bold tracking-widest uppercase">ICON (FontAwesome / Image Path)</label>
                <input 
                  value={editLink.icon || ""}
                  onChange={(e) => setEditLink({...editLink, icon: e.target.value})}
                  className="w-full bg-black/60 border border-white/5 rounded-lg p-4 text-sm outline-none focus:border-neon-blue/50 focus:bg-neon-blue/5 transition-all text-foreground font-mono"
